...
playstore['Reviews'] = playstore['Reviews'].astype('int32')
playstore['Installs'] = playstore['Installs'].astype('int32')
playstore['Size'] = playstore['Size'].astype('int32')
...
def index():
    ...
    stats2 = {
        'best_game' : df8.groupby(by= ['App']).agg({'Reviews': 'sum', 'Rating': 'mean'}).sort_values(by='Reviews', ascending= False).reset_index().iloc[0:5].to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    # Tambahkan hasil result plot pada fungsi render_template()
    return render_template('index.html', stats=stats, result=result, result2=result2, result3=result3, stats2=stats2)
